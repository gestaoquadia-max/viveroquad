...
| `emoji-lib.js` | `EMOJI_LIB` | 14 | **Biblioteca de ícones** (dec. 210): ~480 emojis em 12 categorias, com nomes de busca em português — alimenta o seletor completo dos formulários do admin (os símbolos SVG da casa, `ICON_CAT`, entram como 13ª categoria pelo código) |
| `titulos.js` | `TITULOS_CATALOGO` · `TITULOS_SEED` | 15 | **Títulos e condecorações** (dec. 218): o catálogo do que existe para conquistar (nome, ícone, descrição e `comoGanhar`) e o que o aluno da demonstração já tem. Concessão real virá do servidor |
| `lojas-alunos.js` | `LOJAS_DEMO` | 20 | **Mercado interno** (dec. 216): as lojas de outros alunos — nome, dono, lema, se está aberta e os itens à venda (`item`, `qtd`, `preco` em QdC). Na plataforma real vem do servidor |
| `itens-quest.js` | `ITENS_QUEST` | 15 | **Itens de Quest** (dec. 207): construídos com insumos da mochila (receita `[[id, qtd]…]`) e `efeito` que liga o item ao sistema (`foto` ATIVO; score/QdC/diamante a definir) |
| `itens-combate.js` | `ITENS_COMBATE` | 15 | **11 itens** de mochila (compra repetida, dec. 172) — os 4 últimos são os **insumos da quest do Blindado** (dec. 207/209: liga/pneu/MAG/geo, todos `disp:'drop'` — conquistados resolvendo os pacotes de 10 questões, nunca vendidos). Dois campos de DROP (dec. 194): `disp` = como o aluno obtém (`'venda'` é o padrão e pode ser omitido · `'drop'` = só por sorteio, fora da vitrine · `'ambos'` = vende e também cai) e `drop` = chance do sorteio em % (1–100). Hoje: Cantil com `disp:'ambos'`/`drop:10` e **"Patch da sorte"** (`disp:'drop'`, `preco:0`, `drop:12`), o item raro que só existe no drop — os outros 5 não têm os campos e valem como `venda` |
