...
| 4.8 | **Chaves localStorage reais** (na auditoria): `vq_device_authorized`, `vq_last_sync`, `vq_pending`, `vq_tut_step`, `vq_tut_done`, `vq_tut_rew`, `vq_tut_skip`, `vq_intro_done`. *Atualização 01/08:* as 6 primeiras foram **removidas** com regressão verde (fluxo revogado de dispositivo + chaves de tutorial nunca lidas); vivas em 01/08: só `vq_tut_skip` e `vq_intro_done`; *atualização 03/08 (dec. 196): **3 chaves**, com a nova `vq_evolucao` (DA-10) — e o **bug do reset foi corrigido**, o handler limpa as três* | ~3736–3741, ~5697, ~6882 (monolito 30/07) | CONFIRMADO NO CÓDIGO · resolvido em 01/08 |
