...
3. **O botão "Resetar demonstração" não zera tudo:** ele limpa 7 flags mas esquece `vq_intro_done` (Seção 3) — para um reset completo de verdade é preciso limpar o localStorage manualmente ou corrigir o reset. ***Atualização 03/08/2026 (dec. 196): CORRIGIDO** — o handler limpa as três chaves vivas de hoje (`vq_tut_skip`, `vq_intro_done` e a nova `vq_evolucao`, criada pela DA-10).*
