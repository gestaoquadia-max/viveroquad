...
| `README.md`, `docs/00-comece-aqui-danilo.md`, `docs/03-guia-de-build-e-publicacao.md` | **RECONSTRUIR** (reescrever contra o estado de 28/07) | Congelados em ~18/07: URL antiga do artefato, tour de "9 passos" que não existe, roteiro de demonstração que guia para telas removidas (portão "Minha primeira vez"), build.py invisível, tamanhos irreais (70 KB/4 MB vs 764 KB/9,4 MB), árvore de arquivos com ≥10 omissões. Risco: são justamente os documentos de entrada — hoje desorientam quem chega. |
| `docs/01-visao-e-escopo.md` | REAPROVEITAR COM REFATORAÇÃO | Síntese fiel do PDF; precisa de nota de defasagem (economia ativa no protótipo; cadastro no site, não no app). |
...
5. **Manter o protótipo vivo tem custo baixo e valor alto** (refatorações mínimas do build, higiene da seção 4, docs de entrada reescritos) — ele continua sendo a melhor ferramenta de alinhamento entre o gestor e qualquer time que venha a construir o produto real.
