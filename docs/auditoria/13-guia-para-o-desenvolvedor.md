...
6. **Recrie a rede de testes**: o repositório não tem nenhum teste versionado — os hooks `window.__*` documentam onde as suítes Playwright se apoiavam (56 suítes rodaram fora do repo em 01/08); versione as novas junto do código. *Atualização 01/08 (dec. 190) — SUPERADO: não há o que recriar.* As suítes **já estão versionadas** em `tests/` (**57** hoje, após a rodada de 02/08) e `verify.py` = build + regressão é o portão de aceite oficial; a orientação vigente é **manter e estender** essa rede, nunca refazê-la.
