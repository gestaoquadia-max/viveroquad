...
Fonte de verdade: o protótipo executado (`index.html`, gerado das 20 partes de `src/` + `data/`), conforme apurado nos dossiês A–E (`scratchpad/handoff-inv/`), com apoio de `docs/auditoria/03-fluxos-do-usuario.md` e do `docs/02-registro-de-decisoes.md` (dec. 1–191).
...
- DADO DEMONSTRATIVO: valores +1/+5 e o tamanho de bloco 10 (dec. 185); as 60 cartas do `tr-bank.js` e as 40 do `aula-demo.js` ("Poderes administrativos").
...
- REGRA DE PRODUTO CONFIRMADA: recompensa da noite concluída — botão dourado, +1 Score por bloco + 1 QdC, resgate único por turma (dec. 26).
...
- DADO DEMONSTRATIVO: metas das patentes (900, 3.300, …, 12.500 repetido nas 6 finais — curva provável placeholder), ranking geral fictício (1.286 usuários, aluno fixo em 87º), colegas da sala, "presença 9/10" (texto fixo do HTML).
...
## Etapa 17 — Estornos: 7 dias, e o consumo mata o estorno

Toda compra do aluno aparece no bloco "**Estornos · até 7 dias**" da Loja com contagem regressiva visível (FALTAM 7 DIAS → … → ÚLTIMO DIA → PRAZO ENCERRADO, botão morto). O estorno é **direto, sem aprovação**: dois toques ("Estornar" → "Confirmar estorno?", que expira em 4 s), devolução na **moeda original** e desfazimento da posse por tipo — matrícula devolve a vaga à moeda usada e sai de `MATRICULAS` (se era a turma **em uso**, o app passa à matrícula restante); físico devolve estoque e cancela o pedido; evento/simulado desinscreve e sai da portaria; skin regride a cadeia ou reabre a escolha da farda; a administração é avisada em "Estornos e desistências". E o **consumo mata o estorno**: liberar a entrada na portaria, liberar o inscrito do simulado ou confirmar a entrega do físico marca a compra como consumida e a tira da janela na hora — "participar do aulão e estornar depois lesaria a empresa".

- REGRA DE PRODUTO CONFIRMADA: 7 dias corridos com contagem regressiva e dois toques (RN-26); devolução na moeda original; desfazer por tipo, incluindo vaga e turno liberados (dec. 67/100; RN-28); estorno da turma em uso → app passa à restante (dec. 162; RN-10); consumo mata o estorno pelos três gatilhos (dec. 178); portaria sincronizada (dec. 180); estorno direto, admin apenas informado.
...
- PERGUNTA PENDENTE (dossiê A): a autocorreção silenciosa da turma ativa no vencimento por data é o comportamento desejado? O histórico da turma encerrada fica acessível ao aluno depois do fim (hoje some tudo que era "da turma")? Haverá oferta de renovação automática/desconto?
...
| 11 · Blocos do dia | aba Missões (src/03); `BLOCOS_TURMA`/`renderDia`/`trFimBloco` (src/11 l.29–628) | `vtr.mjs`, `vfc.mjs` |
| 12 · Treinamento Rápido | `#trLayer`; `trMontarFila`/`trPeso`/`trAj` (src/11 l.474–637) | `vtr.mjs` |
...
| 16 · Loja no cotidiano | vitrines (src/03 l.264–366); loja/economia (src/14); mochila/skins (src/15); eventos (src/08/17); simulados (src/11/18) | `vloja*.mjs`, `vmochila.mjs`, `vevento.mjs`, `vsim.mjs` |
| 17 · Estornos | "Estornos · até 7 dias" e Relatório (src/19); consumo nas Liberações (src/17 l.367–550) | `vfased.mjs`, `vk1.mjs`, `vo1.mjs`, `vu1.mjs` |
| 18 · Retorno diário | Atrasadas/`DIA_EXPIRA_DIAS` (src/11 l.29–71); `showView` revalida (src/07 l.370–374) | `vtr.mjs`, `vi1.mjs`, `vr1.mjs` |
| 19 · Encerramento e nova matrícula | `matriculasAtivas`/`turmaAtiva` autocorreção (src/07 l.135–221); `#matLayer` (src/06) | `vfaseb.mjs`, `vturma.mjs`, família `vfase*` |
