...
- Persistência: `localStorage` apenas para `vq_tut_skip` e `vq_intro_done`; sessão/credenciais não persistem (recarregar = deslogado). SIMULAÇÃO LOCAL.
...
- DIVERGÊNCIA: aside "Testar o novo acesso (V0)" (src/06 l.529–537) descreve o fluxo antigo (código por e-mail, 3 cenários, autorização de dispositivo, indicador ONLINE/OFFLINE) que não existe mais; o botão "Resetar demonstração" só limpa `vq_tut_skip` e não limpa `vq_intro_done` (comentário do próprio fonte, src/13 l.801 — bug aberto por decisão, dec. 188).
...
**Simulações atuais:**
- Recompensas e carteiras em memória (não persistem entre recargas, exceto as duas chaves `vq_*`). SIMULAÇÃO LOCAL.
...
- `MATRICULAS` (semente: PATAMO Noite), `turmaAtivaId`, `CONCURSOS` (`data/concursos.js` — 6 concursos com árvore de edital), `MODALIDADES` (nivelamento/regular/questões — RONDESP/PATAMO/BOPE são apelidos, dec. 138), `DB_ALUNO`, `CONTAS`.
...
- **Eventos da semana** (`#evStrip`, src/08 l.46–116): esteira horizontal infinita (3 cópias; arrasto de mouse e roda). Eventos são **do QUAD, não da turma** (dec. 161 — revogou a turma-alvo da dec. 150): todo evento vigente aparece para qualquer aluno. Etiquetas: INSCRITO / LOTADO / EM CHOQUE / NA LOJA / +BÔNUS / tag própria. Pago não comprado: clique leva direto ao item na Loja com destaque piscando (`lojaLevarAte`, l.223–234; dec. 173; ver Módulo 8). Página do evento (`openEvento`, l.237–308): regras de score de carreira e de Quad Coins por evento (arrays `score`/`coins` de `data/eventos.js`), inscrição gratuita com confirmação em 2 toques quando há choque de agenda, link do evento online liberado ao inscrito (toast demo), botão "Garimpar Quad Coins escondidos" (+15 QdC, uma vez — missão de evento). Vencido (`evAcabou`) some do carrossel, da Loja e do calendário. FUNCIONAMENTO FUNCIONAL DO PROTÓTIPO.
...
- Clicar em "NA LOJA" leva ao item exato, centralizado e piscando (dec. 173). REGRA DE PRODUTO CONFIRMADA.
...
**Simulações atuais:** SIMULAÇÃO LOCAL: cronograma embutido (planilha real citada como fonte); relógio local como fonte das faixas; avisos/materiais/eventos vivem em memória JS (nada persiste entre sessões, exceto chaves `vq_*` do tutorial); "aluno inscrito" e ocupação de eventos parcialmente semeados (`ev.ocup`); link do evento online é toast; conectividade é um interruptor demo.
...
**Ações disponíveis:** Responder bloco do dia; recuperar atrasada; abrir Treinamento Rápido (trocar aba Gerais/Específicas, responder 10, avançar o rodízio, recomeçar após zerar); retirar benefícios da noite; iniciar a Introdução no Quad (se nunca feita); sair no meio (bloco não marca feito; sem penalidade implementada).
...
**Simulações atuais:** SIMULAÇÃO LOCAL: nascimento/expiração de blocos por seeds com `Date.now()` (não há agendador de 22h15 nem cron de ondas D+7/D+30); banco de questões único local; progresso zera a cada recarga (memória JS; só `vq_intro_done`/`vq_tut_skip` persistem); hook `__noite.concluirTudo` existe só para testes.
...
- **Questões do treinamento**: cartas `fc(m, a, c, t, o)` — matéria, assunto, gabarito C/E, enunciado, comentário; ver Módulo 5.
- **Classificação questão → árvore do edital**: a ligação é por rótulo textual "matéria + assunto": cada carta carrega `c.m`/`c.a`; `trMontarFila` casa `TR_BANK` contra `EDITAL_ATUAL` (árvore `[matéria, [[assunto, [subassuntos]], …]]` de `data/concursos.js`/editais); o ajuste de Domínio grava em `trAj['concurso|matéria|assunto']` e `edSubPct` soma esse deslocamento ao percentual de cada subassunto do assunto (base determinística por hash — src/07 l.438–449). Materiais usam a mesma etiqueta matéria+assunto (ver Módulo 4). **Não há ID de questão nem vínculo por subassunto** — a granularidade real termina no assunto. FUNCIONAMENTO FUNCIONAL DO PROTÓTIPO · SIMULAÇÃO LOCAL (percentuais do Domínio são sintéticos + deslocamento real do treino).
...
**Simulações atuais:** SIMULAÇÃO LOCAL: validação de pontos no front ("a interface nunca decide sozinha… no sistema real vem validado do servidor" — src/07 l.39–40); botão "dia de estudo (simulação)"; botão "Liberar nova tentativa (simulação)"; botão demo de subir patente; ranking geral e colegas fictícios; posição fixa no geral; barras do Domínio por hash; bloqueio de 24h sem relógio persistente (`provaBloqueadaAte` em memória — recarregar a página zera tudo).
...
**Atalho "NA LOJA" (dec. 173)** — comprovado: evento pago no carrossel do Início tem tag "NA LOJA"; o clique abre a Loja direto no item, centralizado e piscando (classe `achei`, `lojaLevarAte`). Vale também para simulados presenciais ("a inscrição é a compra da vaga na Loja"). Ver Módulo 4.
...
**Nome do módulo:** Compras, matrículas e estornos — pós-venda da Quad Store (blocos "Estornos · até 7 dias" e "Relatório de compras" do aluno; painéis "Compras" e "Estornos e desistências" do admin; recepção/portaria nas Liberações). Complementa a vitrine descrita no Módulo 8 (ver Módulo 8).
...
**Ações disponíveis:**
- Aluno: "Estornar" → "Confirmar estorno?" (dois toques); trocar período do relatório; nenhuma ação sobre linhas consumidas/encerradas.
...
**Simulações atuais:**
- SIMULAÇÃO LOCAL — devolução de Dmn é só crédito local (o estorno financeiro real é bancário/gateway); `quando: 'agora'`/"há N dias" são rótulos fixos; a portaria "reconhece" o aluno por `euSou` na sessão única; inscritos-semente são fictícios; nada persiste entre sessões.
...
- REGRA DE PRODUTO CONFIRMADA — gratuito pode premiar (selo +BÔNUS quando há regras); pago vende em QdC ou Dmn (uma moeda por evento).
...
**Perfis envolvidos:**
- Administrador (N.P.P.): há **1 perfil** administrativo na demo; a UI diz "no sistema real o cadastro de administrador é aprovado pela direção".
...
- **Gate:** exige e-mail com "@" e chave `NPP-2026` (case-insensitive). DIVERGÊNCIA (conhecida e reconfirmada ao vivo — sonda F3 nº1): **o e-mail não é validado** — `qualquer@coisa.com` + chave entra; `npp@quadconcursos.com.br` é só placeholder. Comentário no código: "[INTEGRAÇÃO REAL] a chave é emitida e revogada pela direção, por pessoa".
- **Abas/views (8 no total):** navbar com 5 (`v-adm-controle`, `v-adm-hoje`, `v-adm-alunos`, `v-adm-liber`, `v-adm-loja`) + 3 auxiliares por botão (`v-turmas` Estrutura, `v-edital`, `v-questoes`). **Não existe aba/tela de "Configurações"** — inexistente no protótipo. Cada área "abre limpa" só com os atalhos por bloco (dec. 127/113).
- **Painel de controle (`v-adm-controle`):** FUNCIONAMENTO FUNCIONAL DO PROTÓTIPO —
  ...
  - Crédito manual QdC/Dmn com motivo e histórico; conta da demo credita na carteira na hora.
...
**O que precisará de implementação real:** perfis administrativos múltiplos com emissão/revogação de chave por pessoa (e validação do e-mail); persistência/banco para todos os cadastros; upload/parse de edital e PDFs; mensageria/push; ledger de moedas; check-in de portaria; geração real de QR e PDF; chave primária de docente que não seja o nome (homônimos).
...
**Perguntas pendentes:**
- PERGUNTA PENDENTE — quantos perfis/papéis administrativos existirão (N.P.P., direção, recepção?) e com quais permissões?
- PERGUNTA PENDENTE — o e-mail do admin passa a ser credencial validada?
