...
- **Conteúdo:** persistir banco de questões com ID por questão, gabarito, tags pela árvore do edital e campo de uso; árvores de edital **versionadas com relação ao concurso**; cronograma por turma com histórico de trocas; materiais com relação turma×matéria×assunto e arquivo em storage.
  - ***Contrato banco de questões → Domínio (dec. 202/203):*** para os botões de play do Domínio acenderem com o banco real, cada questão precisa chegar classificada por **matéria + assunto com os nomes EXATOS da árvore do edital vigente** (o casamento é textual — DA-02 prevê a troca por IDs na modelagem) e, opcionalmente, **subassunto** (refina a ordem do baralho da revisão; questão sem subassunto vale como "do assunto inteiro" e completa os blocos). **Nenhuma lista no código decide quais subassuntos têm treino**: o botão acende pela simples presença de questões daquele assunto no banco — comprovado em runtime pela suíte `vy1` (as cartas da aula migradas para o banco acendem os 8 subassuntos de Poderes administrativos na hora, sem qualquer alteração de código).
