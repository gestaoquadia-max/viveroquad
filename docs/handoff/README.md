...
> ⚠️ **AVISO DE VIGÊNCIA** — este pacote foi **extraído em 02/08 contra as decisões 1–191**; as **decisões 192–194** (mesma data) foram **incorporadas nesta revisão de 03/08** — consulte o CHANGELOG para mudanças posteriores.
>
> O que a revisão trouxe: **dec. 192** (o evento presencial inscrito não some do calendário ao vencer — vira CONCLUÍDO ou FALTOSO — e o evento realizado sai da janela de estorno), **dec. 193** (a compra feita no tutorial nasce fora da regra dos 7 dias) e **dec. 194** (sistema de DROP: itens de combate conquistados ao resolver questões). Contagens vigentes: **57 suítes** em `tests/` (nova `vv1.mjs`), **35 hooks** `window.__*`, **7 itens** de combate em `data/itens-combate.js`.
...
Cada afirmação dos documentos está classificada com os rótulos padrão do projeto (FUNCIONAMENTO FUNCIONAL DO PROTÓTIPO · SIMULAÇÃO LOCAL · REGRA DE PRODUTO CONFIRMADA · DADO DEMONSTRATIVO · DECISÃO POSTERIOR · FUNCIONALIDADE PLANEJADA · DIVERGÊNCIA · PERGUNTA PENDENTE) e rastreada até o código (`src/NN`), as decisões numeradas (`docs/02-registro-de-decisoes.md`, dec. 1–194) e sondas Playwright executadas sobre o build.
...
| 04 | [Decisões posteriores e divergências](./04-decisoes-posteriores-e-divergencias.md) | O que modifica o que existia antes: 30 fichas de decisões posteriores (seção A — A28/A29/A30 cobrem as dec. 192/193/194) e 28 fichas de divergências confirmadas (seção B), com fonte, impacto e "precisa ajustar?" |
...
| 06 | [Guia técnico do protótipo](./06-guia-tecnico-do-prototipo.md) | Como rodar, construir (`build.py`/`verify.py`), inspecionar e testar o protótipo: as 20 partes de `src/`, os seeds de `data/`, as 57 suítes e os hooks de teste |
...
2. **As 57 suítes Playwright versionadas em `tests/`**: a rede de regressão que prova esse comportamento, com `verify.py` (build + regressão completa) como **portão de aceite oficial** (dec. 190) e os hooks `window.__*` como contrato de teste.
