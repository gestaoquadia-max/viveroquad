...
**Fonte de verdade funcional: o protótipo navegável** (`/home/user/viveroquad`, fonte em `src/`, build `index.html`) **+ as 58 suítes de regressão em `tests/`** (dec. 190). Apoio documental: `docs/arquitetura/00-arquitetura-oficial.md` (Consolidação v1.0, hierarquia máxima), `docs/arquitetura/02-decisoes-administrativas.md` (DA-01…DA-10, 03/08), `docs/02-registro-de-decisoes.md` (dec. 1–197), `docs/auditoria/` (14 docs, 30/07/2026) e o relatório rev. 2.3 (documento-base do produto, pendente de revisão desde a dec. 11).
...
- **DROP de itens.** Concluir um bloco de 10, o treinamento rápido ou um simulado digital **sorteia** itens de combate pela chance configurada em cada um: o conquistado cai na mochila sem custo e sem estorno, anunciado por uma celebração dourada — há inclusive item **raro, que só existe no drop** e nunca aparece à venda (dec. 194). O drop não dispara no tutorial. REGRA DE PRODUTO CONFIRMADA (mecânica); as chances são DADO DEMONSTRATIVO.
- **Registro do que foi cumprido.** No calendário, o evento presencial em que o aluno se inscreveu não desaparece ao passar: vira **CONCLUÍDO** (entrada liberada na portaria) ou **FALTOSO** (o dia passou sem registro) — o histórico de constância fica visível (dec. 192). REGRA DE PRODUTO CONFIRMADA.

FUNCIONALIDADE PLANEJADA nesta frente: Quests/forja de itens, Guarnições/GvG/PvP (V2), "Prestígio" pós-Coronel, Intendência plena — anúncios bloqueados na interface, sem mecânica. (O **drop**, antes nesta lista, saiu dela: virou mecânica implementada pela dec. 194.)
...
No protótipo, o portão do admin valida apenas a chave `NPP-2026` — o e-mail aceita qualquer coisa com `@`. DIVERGÊNCIA conhecida (a intenção declarada é chave emitida e revogada pela direção, por pessoa) + SIMULAÇÃO LOCAL. **RESPONDIDA em 03/08 pela DA-08** (dec. 195): existem **diferentes perfis administrativos**, cada um com permissões conforme a sua responsabilidade — o protótipo passou a demonstrar quatro (Direção, Coordenação pedagógica, Recepção e Financeiro), que filtram as abas visíveis e assinam as ações no extrato (dec. 196). PERGUNTA PENDENTE remanescente: o **RBAC fino** e a chave/identidade **por pessoa**.
...
- **Quase nada persiste — e o que persiste é do dispositivo, não da conta.** Recarregar a página (F5) apaga compras, pedidos, matrículas compradas, inscrições, mensagens e tudo que o admin criou na sessão. **Desde 03/08 (dec. 196, DA-10) sobrevivem 3 chaves de `localStorage`**: `vq_tut_skip`, `vq_intro_done` e **`vq_evolucao`** — esta última devolve, ao reabrir o app, os saldos de Quad Coins e Diamantes, o score de carreira, a mochila, as skins possuídas, o avatar, o nome de guerra, a turma ativa, o ajuste do Domínio e os 60 últimos lançamentos do extrato. Continua não havendo conta, sessão nem servidor: é continuidade **por aparelho**, e um navegador diferente começa do zero.
...
- **A especificação é a experiência.** O que o protótipo *faz* — cada fluxo, validação, toast, regra de bloqueio — é a especificação funcional que o engenheiro deve honrar. Ela é acompanhada de uma **especificação executável**: as **58 suítes Playwright versionadas em `tests/`**, com `verify.py` (build + regressão) como portão de aceite oficial (dec. 190) e **42 hooks** `window.__*` como contrato de teste. Em conflito entre documentos, vale a hierarquia: `docs/arquitetura/00-arquitetura-oficial.md` → registro de decisões → auditoria → rev. 2.3; em dúvida sobre comportamento, **vale o código e as suítes**.
- **O protótipo já demonstra o ledger e a continuidade — o que falta é o back-end, não a demonstração.** Desde a dec. 196, toda entrada e saída de moeda vira **lançamento** com tipo, origem, destino, **autor** e **saldo resultante** (DA-03), lido pelo aluno no card "Carteira · extrato e compras"; a evolução do aluno **volta** depois do F5 (DA-10); a área administrativa tem **4 perfis** que filtram as abas e assinam o crédito manual (DA-08); a turma encerrada fica **ARQUIVADA** com histórico consultável (DA-06). Nada disso é servidor: é o **comportamento aprovado**, demonstrado localmente, que a aplicação real terá de entregar de verdade — pela conta, com transação e auditoria.
- **O que falta é o produto inteiro.** Back-end, banco de dados, autenticação/RBAC, **ledger auditável no servidor**, pagamentos, tempo real multiusuário, extração de PDFs, telemetria, notificações, **persistência por conta** — cada módulo interno da Consolidação v1.0 é "Módulo Planejado" até ganhar especificação aprovada. **É proibido implementar módulo sem especificação aprovada e descrever módulo planejado como implementado** (arquitetura oficial, §8).
