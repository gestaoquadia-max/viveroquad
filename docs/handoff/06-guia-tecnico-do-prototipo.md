...
> **Revisão de 03/08/2026** — o guia foi escrito contra o estado de 02/08 (dec. 1–191). As decisões **192–194** da mesma data foram incorporadas: nova suíte `vv1.mjs`, hooks `__dropRng`, `__dropSortear`, `__calRefresh`, `__estRefresh` e o sistema de DROP na parte 15.
>
> **Segunda revisão de 03/08/2026** — incorporadas as dec. **195–197**: **58 suítes** (nova `vw1.mjs`), **42 hooks** `window.__*` (os 7 novos: `__ledger`, `__ids`, `__carteira`, `__admPerfil`, `__evolSalvar`, `__evolCarregar`, `__evolMarcar`), **3 chaves** de `localStorage` (nova `vq_evolucao`), ledger da carteira nas partes 07/19, perfis administrativos na 16, persistência na 20 e o card unificado "Carteira · extrato e compras" na Loja.
...
O 1º acesso do aluno dispara o tutorial obrigatório de 29 passos (botão "‹ Pular" disponível; a flag `vq_tut_skip` no localStorage também pula) — **desde a dec. 197 o balão não mostra mais o contador "N / 29"**, só o nome "QUAD". O botão "Reiniciar demonstração (dispositivo)" (painel lateral) **limpa as três chaves** `vq_tut_skip`, `vq_intro_done` e `vq_evolucao`: o bug que deixava `vq_intro_done` para trás foi **corrigido na dec. 196** (src/13).

## 3. Como verificar

```sh
python3 verify.py            # build + as 58 suítes de regressão
...
| 07-js-estado-dados | Abertura do `<script>`/IIFE, estado global, moedas, docentes, turmas, matrículas, concursos; **`LEDGER`/`ledgerLancar`/`ledgerOp`** (DA-03), **`novoId()`/`MEU_ID`** (DA-01) e **`matriculasArquivadas()`** (DA-06) |
...
| 12-js-gamificacao-perfil | Insígnias, rankings/Quadrômetro, promoções, simulado digital, central de tutoriais, **bloco "Minhas turmas · ativas e arquivadas"** com a etiqueta ARQUIVADA (DA-06) |
| 13-js-admin-estrutura | Estrutura/Domínio, criação de turmas e isoladas, banco de professores, reset da demo (**limpa as 3 chaves `vq_*`**) |
...
| 16-js-admin-controle | Gate N.P.P. com **`ADM_PERFIS`** (4 perfis, `admPodeVer`/`admPerfilNome` — DA-08), contas/créditos (**crédito manual assinado no ledger**), mensagens por público, gift cards, cronograma, materiais |
...
| 19-js-compras-estornos | **Carteira do aluno**: card unificado "Carteira · extrato e compras" (`renderRelCompras` + `renderExtrato`, KPIs recebido/gasto/compras/a receber, saldo de abertura `MV-00000`/`MV-00001`) e a janela de estornos de 7 dias |
| 20-js-relatorios-boot | Relatórios com gráficos, **persistência DA-10** (`evolSalvar`/`evolCarregar`/`evolMarcar`, chave `vq_evolucao`, debounce 400 ms), cascata de inicialização, fechamento do IIFE |
...
58 suítes Playwright headless, construídas junto com o produto, que verificam o comportamento de ponta a ponta (aluno, professor, admin). São o **critério de aceite oficial**. Regras de manutenção (de `tests/README.md`):
...
3. As suítes conversam com o protótipo pelos **42 ganchos `window.__*`** do fonte (`__mat`, `__turmaAtiva`, `__eventos`, `__evLotar`, `__noite`, `__introFeita`, `__simDig`…; os 4 das dec. 192/194: `__dropRng` — RNG determinístico do drop —, `__dropSortear`, `__calRefresh` e `__estRefresh`; e os 7 da dec. 196: `__ledger`, `__ids`, `__carteira`, `__admPerfil`, `__evolSalvar`, `__evolCarregar` e `__evolMarcar`) — **contrato estável**: não remova um gancho sem atualizar o README dos testes.
...
| M3 Conta, matrículas e turma ativa | 07 (`MATRICULAS`/`definirTurmaAtiva`/`matriculasArquivadas`), 12 (UI de troca, turmas arquivadas), 14 (compra de turma) | `turmas-loja`, `concursos` | `vturma`, `vfase*`, `vt1`/`vq1`/`vr1`, **`vw1`** (turma arquivada) |
...
| M9 Gift cards e carteira | 07 (resgate, **`LEDGER`/`ledgerLancar`**), **19 (extrato: `renderExtrato`, saldo de abertura)**, **20 (persistência da carteira)**, 16 (lotes/QR/crédito manual assinado) | — (`GIFT_CARDS`/`GIFT_LOTES`/`LEDGER` no código) | `vdmn`, `vg1`–`vg3`, `vcontrole`, **`vw1`** (ledger/extrato) |
| M10 Compras, matrículas e estornos | 14 (log/estorno), **19 (card unificado "Carteira · extrato e compras" + janela de estorno)**, 17 (consumo: portaria/entrega), 18 (`semEstorno` no log), 16 | `itens-presenciais`, `turmas-loja`, `simulados` | `vloja*`, `vevento`, `vu1` (portaria), `vv1` (tutorial/evento realizado), **`vw1`** (carteira unificada) |
...
| M16 Área administrativa (N.P.P.) | 05 (chip do perfil), 06 (seletor `#admPerfil` no portão), 13, **16 (`ADM_PERFIS`)**, 17, 18, 20 | praticamente todos os conjuntos | `vadmin`, `vinterno`, `vcontrole`, `vestrutura`, `vrelatorios`, `vk1`, `vu1`, **`vw1`** (perfis administrativos) |
...
- **Persistência mínima (dec. 196)**: recarregar (F5) zera compras, pedidos, matrículas compradas, inscrições e tudo que o admin criou na sessão; **sobrevivem 3 chaves de localStorage** — `vq_tut_skip`, `vq_intro_done` e `vq_evolucao`, esta última devolvendo saldos, carreira, mochila, skins, avatar, nome de guerra, turma ativa, `trAj` e os 60 últimos lançamentos do extrato (DA-10). É persistência **por dispositivo**, não por conta.
- **~9,4 MB por saída**: mídias em base64 (+33%), sem streaming/cache granular — o navegador engole tudo antes do primeiro paint.
- **42 hooks `window.__*` expostos** no artefato publicado (contrato das suítes) + gabaritos das questões nos objetos JS do cliente — agravante desde a dec. 196: o que for forjado pelo console também **persiste** em `vq_evolucao`.
...
- **LGPD**: todos os dados pessoais do protótipo (nomes, telefones, e-mails de `DB_ALUNO`, `DOCENTES`, `CONTAS`) são **fictícios** — nenhum dado real deve entrar no protótipo nem nos seeds de `data/`. **Desde a dec. 196 há dado pessoal (fictício) gravado no dispositivo**: a chave `vq_evolucao` guarda nome de guerra, avatar, saldos e extrato — no produto, o dono do dado é o servidor (DA-07) e qualquer cache local precisa de base legal, expiração e limpeza no logout (DA-09). Retenção/eliminação de dados, privacidade do ranking (top 10 sempre visível, sem opt-out) e o controle de acesso real a materiais são perguntas abertas registradas na auditoria (docs 06/13) que a implementação real terá de responder.
